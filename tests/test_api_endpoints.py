...
def build_multipart_formdata(filename, content, fields, boundary="----PalmMindTestBoundary"):
    body_parts = []
...
def test_endpoint_upload(filename, content, fields, strategy):
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as res:
            ...
            assert "strategy_used" in res_json, "Missing 'strategy_used' key in response"
            assert "chunks_count" in res_json, "Missing 'chunks_count' key in response"
            assert res_json["strategy_used"] == strategy, f"Expected strategy {strategy}, got {res_json['strategy_used']}"
            
            print(f"[PASSED] Ingestion successful! Document ID: {res_json['document_id']}, Chunks: {res_json['chunks_count']}")
            return True
...
def test_endpoint_chat(payload, test_name):
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            ...
            assert isinstance(res_json["history"], list), "'history' must be a list of turns"
            
            print(f"[PASSED] Chat turns validated. Session ID: {res_json['session_id']}")
...
if __name__ == "__main__":
    ...
    print("Please make sure your FastAPI app is running (e.g. uvicorn app.main:app) before calling this runner.")
