    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if status == 200:
                ...
                print("Usage Stats:")
                print(f"  Prompt tokens: {res_json.get('usage', {}).get('prompt_tokens', 'N/A')}")
                print(f"  Completion tokens: {res_json.get('usage', {}).get('completion_tokens', 'N/A')}")
                print(f"  Total tokens: {res_json.get('usage', {}).get('total_tokens', 'N/A')}")
                return True
            else:
