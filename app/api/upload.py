...
async def upload_file(
    ...
    chunk_strategy: str = Form(
        "recursive", description="Options: 'recursive' or 'semantic'"
    ),
    chunk_size: int = Form(
        500, description="Used in recursive chunking character count limit"
    ),
    chunk_overlap: int = Form(
        50, description="Used in recursive chunking overlap size"
    ),
    similarity_percentile: float = Form(
        90.0, description="Used in semantic chunking transition thresholds"
    ),
